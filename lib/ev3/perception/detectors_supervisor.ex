defmodule Ev3.DetectorsSupervisor do
	@docmodule "Supervisor of dynamically started detectors"

	@name __MODULE__
	use Supervisor
	alias Ev3.Detector
	require Logger

	def start_link() do
		Logger.info("Starting #{@name}")
		Supervisor.start_link(@name, [], [name: @name])
	end

	def init(_) do
		children = [worker(Detector, [], restart: :permanent)]
		supervise(children, strategy: :simple_one_for_one)
	end

	def start_detector(sensor, sense) do
		Logger.info("Starting Detector on sensor #{sensor.path} for #{inspect sense}") 
		{:ok, _pid} = Supervisor.start_child(@name, [sensor, sense])
	end
	
end
