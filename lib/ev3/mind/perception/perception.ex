defmodule Ev3.Perception do
	...
	def collision() do
	  ...
	  (%Percept{sense: :proximity, value: n}, history) when n < 10 ->
				if not any_percept?(history, :proximity, 1000, fn(value) -> value > 10 end) do
					Percept.new(sense: :collision, value: :imminent)
				else
					nil
				end
		(%Percept{sense: :proximity, value: val}, history) ->
		    approaching? = latest_percept?(history, :proximity, fn(previous) -> val < previous  end)
		    proximal? = all_percepts?(history, :proximity, 5000, fn(value) -> value < 50 end)
		...
		(%Percept{sense: :touch, value: :pressed}, history) ->
					imminent? = any_percept?(history, :collision, 5000, fn(value) -> value == :imminent end)
					if imminent? do
